...
const state = {
  ...
  filters: {
    ...
    inventoryGrouping: "component", // 'component' = DC → Log Source → Channel hierarchy (default); 'name' = group every channel under its log-source name; 'picker' = chunk 18 flat list with bulk enable
    customLsCompFilter: "", // chunk 13: filter for the component picker on the manual-entry form
    invPickerFilter: "", // chunk 18: filter for the picker view
  },
...
// chunk 18: picker view filter + bulk enable / disable
...
function renderInventory() {
  ...
  // chunk 18: live cascade strip + show picker controls only when picker mode is active.
  ...
  if (state.filters.inventoryGrouping === "picker") {
...
// activate, how many detection strategies light up, and how many
// techniques the chain covers. Updates on every render.
...
// score select (only when enabled), comment input, and a
// component-fed badge. Bulk-enable / bulk-disable via the controls
// at the top of the panel. Custom (user-added) tuples render in
// dedicated .picker-row.custom rows so they're editable from here too.
function renderInventoryPicker(root, lsScores, compScores) {
  ...
  const renderRow = (name, channel, savedEntry, isCustom) => {
    const enabled = savedEntry ? savedEntry.enabled !== false : false;
    const score = savedEntry ? clampInt(savedEntry.score) : 0;
    const comment = savedEntry?.comment || "";
    const customFeeds = (savedEntry?.component_refs || []).length;
    const compFeeds = isCustom ? customFeeds : (state.attack.logSources.find(l => l.name === name && l.channel === channel)?.componentIds?.length || 0);
  ...
  for (const ls of rows) {
    ...
    html += renderRow(ls.name, ls.channel, saved, false);
  }
  for (const e of customRows) {
    html += renderRow(e.name, e.channel, e, true);
  }
